...
- Learn how to source external scripts to run within a Quarto document to modularize your code.
...
Okay, remember how we changed the file path in the code chunk we copied into our document to correct the relative path for reading the data? This happened because in Quarto documents, the working directory is set to the location of the qmd document, but our code is in a different folder. This means they have different relative paths to the data we want to work with. We can simplify things by designating the working directory for our document relative to the root project directory instead. This lets all files work relative to the project root rather than each other, standardizing relative paths. Note that this is mainly important for code chunks - text portions of Quarto documents are not affected. This will clarify some of our confusion with relative paths across our R project.

So, one should learn how to accomplish that by changing the working directory in the project-level YAML settings. So we'll navigate to our \_quarto.yml document in our root project directory and open it.

We'll add the following settings to the YAML file:
...
We're just going to test the exact figure again to verify that this new method works.

### Add the code to our Quarto document

In the heart rate analysis code chunk, delete the code we copied and pasted.
...
Our plot should look the same as the first one we copied and pasted.
...
## Your turn! Create Figure 3 with the external code
...
# Display the plot created by the code in hormone_analysis.R
plot 
```
