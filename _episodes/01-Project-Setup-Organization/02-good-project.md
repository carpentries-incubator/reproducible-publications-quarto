...
> >    - **/processed:** contains a CSV file produced by the `01_data_preprocessing.R` script.
...
> > In the `2022-01-01_data_analyses.R`, `-` is used for a date delimiter between year, month, and day. `_` is used between the rest. This allows us to split on `_`, which would preserve the date (separate from other file info).
> > 
